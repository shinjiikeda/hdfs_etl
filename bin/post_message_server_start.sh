...
DAEMON_OPT="--daemon"
ETL_OPTS="$DAEMON_OPT --logfile $LOG_DIR/post_message.log --conf $BIN_DIR/../conf/config.rb"
...
bundle exec jruby $JRUBY_OPTS $BIN_DIR/post_message_to_kafka.rb $ETL_OPTS >>$LOG_DIR/post_message.out 2>&1 &
if [ $? == 0 ]; then
  echo $! > $PID_FILE
  echo "success"
else
